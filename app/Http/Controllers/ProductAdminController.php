...
class ProductAdminController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'animal_image_url' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);

        if ($request->hasFile('animal_image_url')) {
            $path = $request->file('animal_image_url')->store('products', 'public');
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('animal_image_url')) {
            ...
            if ($product->animal_image_url && \Storage::disk('public')->exists($product->animal_image_url)) {
                \Storage::disk('public')->delete($product->animal_image_url);
            }
            $path = $request->file('animal_image_url')->store('products', 'public');
            $data['animal_image_url'] = $path;
        }
