...
@section('title', 'Track Order ' . $order->display_id)
...
<div class="admin-track-page">
    ...
    <div class="at-header">
        <div class="at-header-left">
            <a href="{{ route('admin.orders.show', $order->id) }}" class="at-back-btn">← Back to Order</a>
            <h1>📦 Track {{ $order->display_id }}</h1>
        </div>
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
            <span class="at-order-badge at-badge-status">{{ $order->status_label }}</span>
            <span class="at-order-badge {{ $order->isLocal() ? 'at-badge-local' : 'at-badge-courier' }}">
                {{ $order->isLocal() ? '🛵 Local Delivery' : '📦 Courier' }}
            </span>
        </div>
    </div>
    ...
    <div class="at-grid">
        ...
        <div>
            <div class="at-card">
                ...
                <div class="at-eta-grid">
                    ...
                    <div class="at-eta-cell">
                        ...
                        <div class="at-eta-value" style="font-size:14px;">{{ $order->isLocal() ? 'Local' : 'Courier' }}</div>
                        <div class="at-eta-sub">{{ $order->shipping_type }}</div>
                    </div>
                </div>

                {{-- Progress --}}
                <div class="at-progress">
                    <div class="at-progress-fill" id="progressBar" style="width: {{ $order->getDeliveryProgressPercent() }}%;"></div>
                </div>

                {{-- Rider --}}
                @if($order->rider_id)
                <div class="at-rider-bar">
                    <div>🛵 <span class="at-rider-name">{{ optional($order->rider)->full_name ?? 'Rider' }}</span></div>
                    <div class="at-rider-status">
                        @if($order->rider_picked_up_at) On the way to customer
                        @else Heading to store for pickup @endif
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="at-card">
                ...
                <div class="at-item">
                    @if($oi->product)
                        <img class="at-item-img" src="{{ $oi->product->image_url }}" alt="" onerror="this.style.display='none'">
                    @endif
                    <div>
                        <div class="at-item-name">{{ optional($oi->product)->product_name ?? 'Product #'.$oi->product_id }}</div>
                        <div class="at-item-meta">Qty: {{ $oi->quantity }} · ₱{{ number_format((float)$oi->unit_price, 2) }}</div>
                    </div>
                    <div class="at-item-price">₱{{ number_format((float)$oi->total_amount, 2) }}</div>
                </div>
                @endforeach
                <hr style="border:0; border-top:1px solid #f0f0f0; margin:12px 0;">
                <div class="at-total-row"><span class="at-detail-label">Subtotal</span><span>₱{{ number_format((float)$order->order_amount, 2) }}</span></div>
                <div class="at-total-row"><span class="at-detail-label">Shipping</span><span>{{ $order->shipping_fee > 0 ? '₱'.number_format((float)$order->shipping_fee, 2) : 'FREE' }}</span></div>
                @if($order->discount_amount > 0)
                <div class="at-total-row"><span class="at-detail-label" style="color:#43a047;">Discount</span><span style="color:#43a047;">-₱{{ number_format((float)$order->discount_amount, 2) }}</span></div>
                @endif
                <div class="at-total-row grand">
                    <span>Total</span>
                    <span class="at-total-amount">₱{{ number_format((float)$order->total_amount, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="at-card">
                ...
                <div class="at-timeline">
                    @foreach($flow as $idx => $stepKey)
                    @php
                        $isDone = $idx < $currentIdx;
                        $isCurrent = $idx === $currentIdx;
                        $label = \App\Models\Order::STATUS_LABELS[$stepKey] ?? ucfirst(str_replace('_', ' ', $stepKey));
                    @endphp
                    <div class="at-tl-step {{ $isDone ? 'done' : '' }} {{ $isCurrent ? 'current' : '' }}">
                        <div class="at-tl-dot">{{ $isDone ? '✓' : ($isCurrent ? '●' : '') }}</div>
                        <div class="at-tl-title">{{ $label }}</div>
                        @if($isDone || $isCurrent)
                            @php
                                $historyEntry = $order->statusHistory->where('status', $stepKey)->first();
                            @endphp
                            @if($historyEntry)
                                <div class="at-tl-time">{{ $historyEntry->created_at->format('M j, g:i A') }}</div>
                                @if($historyEntry->note)
                                    <div class="at-tl-desc">{{ $historyEntry->note }}</div>
                                @endif
                            @endif
                        @endif
                    </div>
            ...
            <div class="at-card">
                ...
                <div class="at-detail-row">
                    ...
                    <span class="at-detail-value">{{ optional($order->user)->full_name ?? '—' }}</span>
                </div>
                <div class="at-detail-row">
                    <span class="at-detail-label">Email</span>
                    <span class="at-detail-value">{{ optional($order->user)->email ?? '—' }}</span>
                </div>
                <div class="at-detail-row">
                    <span class="at-detail-label">Phone</span>
                    <span class="at-detail-value">{{ optional($order->shippingAddress)->phone_number ?? '—' }}</span>
                </div>
                ...
                <div class="at-detail-row">
                    ...
                    <span class="at-detail-value" style="text-align:right; max-width:200px;">
                        ...
                        {{ $order->shippingAddress->barangay ? $order->shippingAddress->barangay . ', ' : '' }}
            ...
            <div class="at-card">
                ...
                <div class="at-detail-row">
                    ...
                    <span class="at-detail-value">{{ $order->order_id }}</span>
                </div>
                <div class="at-detail-row">
                    <span class="at-detail-label">Date</span>
                    <span class="at-detail-value">{{ $order->created_at->format('M j, Y g:i A') }}</span>
                </div>
                <div class="at-detail-row">
                ...
                <div class="at-detail-row">
                    ...
                    <span class="at-detail-value" style="font-family:monospace; letter-spacing:1px;">{{ $order->tracking_number }}</span>
                </div>
                @endif
                @if($order->rider)
                <div class="at-detail-row">
                    <span class="at-detail-label">Rider</span>
                    <span class="at-detail-value">{{ $order->rider->full_name }}</span>
                </div>
                @endif
            </div>
        </div>
...
(function() {
    ...
    var riderPos = { lat: {{ $rLat }}, lng: {{ $rLng }} };
    var hasRider = {{ $order->rider_id ? 'true' : 'false' }};
    ...
    var TRACKING_URL = '{{ route("admin.orders.tracking-data", $order->id) }}';
