                ...
                <tr>
                    ...
                    <td class="col-actions">
                        <button type="button" class="action-btn" title="Edit" aria-label="Edit category"
                            onclick="openCategoryModal('edit', {{ json_encode([
                                'id' => $category->id,
        ...
        <table class="orders-table" id="animalTypesTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color: #888; font-size: 13px;">{{ $at->description ?? '—' }}</td>
                    <td>{{ $at->product_count }}</td>
                    <td>{{ $at->sort_order ?? 0 }}</td>
                    <td>
                        <span style="padding: 2px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; {{ ($at->status ?? 'Active') === 'Active' ? 'background: #e9f2ea; color: #2E6C34;' : 'background: #fef3cd; color: #856404;' }}">
                            {{ $at->status ?? 'Active' }}
                        </span>
                    </td>
                    <td class="col-actions">
                        <button type="button" class="action-btn" title="Edit" onclick="openAnimalTypeModal('edit', {{ json_encode([
                            'id' => $at->id,
                            'name' => $at->animal_type,
                        ...
                        @if(($at->status ?? 'Active') === 'Active')
                        <button type="button" class="action-btn" title="Draft" onclick="toggleAnimalStatus({{ $at->id }})">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
                        </button>
                        @else
                        <button type="button" class="action-btn" title="Restore" onclick="toggleAnimalStatus({{ $at->id }})" style="color: #22c55e;">
                        ...
                        <button type="button" class="action-btn" title="Delete" onclick="deleteAnimalType({{ $at->id }})" style="color: #ef4444;">
                            <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M16 9v10H8V9h8m-1.5-6h-5l-1 1H5v2h14V4h-4.5l-1-1zM18 7H6v12c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7z"/></svg>
                        </button>
                        @endif
                    </td>
