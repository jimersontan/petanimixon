...
<div
    id="{{ $widgetId }}"
    class="pmn-root pmn-theme-{{ $theme }}"
    data-pmn-root
    data-limit="{{ $limit }}"
    data-empty-message="{{ $emptyMessage }}"
    style="--pmn-width: {{ $panelWidth }}; --pmn-right: {{ $panelRight }};"
>
    <button
        type="button"
        class="pmn-trigger {{ $buttonClass }}"
        data-pmn="trigger"
    ...
    (function () {
        ...
        if (!window.PetMarktNotifications) {
            ...
            function refreshBadge(widget) {
                return request('{{ route("notifications.unread") }}').then(function (data) {
            ...
            function load(widget) {
                widget.refs.list.innerHTML = '<div class="pmn-empty"><strong>{{ $loadingMessage }}</strong></div>';

                let url = '{{ route("notifications.list") }}?limit=' + widget.limit;
            ...
            function markAll(widget) {
                return request('{{ route("notifications.mark-all-read") }}', {
                    method: 'POST',
                    body: JSON.stringify({})
                }).then(function () {
                    return load(widget);
...
@if($loadHookName || $refreshHookName || $setBadgeHookName)
    <script>
        (function () {
            function assignHooks() {
                @if($loadHookName)
                    window['{{ $loadHookName }}'] = function () {
                        return window.PetMarktNotifications.refreshById('{{ $widgetId }}');
                    };
                @endif

                @if($refreshHookName)
                    window['{{ $refreshHookName }}'] = function () {
                        return window.PetMarktNotifications.refreshBadgeById('{{ $widgetId }}');
                    };
                @endif

                @if($setBadgeHookName)
                    window['{{ $setBadgeHookName }}'] = function (count) {
                        return window.PetMarktNotifications.setBadgeCountById('{{ $widgetId }}', count || 0);
                    };
                @endif
            }
