{{-- In-app notification bell + dropdown (session auth; CSRF on mutating requests) --}}
...
(function() {
    ...
    function loadUserNotifications() {
        notificationFetch('{{ route("notifications.list") }}?limit=5')
    ...
    function checkNewNotifications(newCount) {
        if (previousUnreadCount !== -1 && newCount > previousUnreadCount) {
            notificationFetch('{{ route("notifications.list") }}?limit=1&unread_only=true')
    ...
    function updateUserNotificationBadge() {
        notificationFetch('{{ route("notifications.unread") }}')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (markAllReadBtn) {
            markAllReadBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                notificationFetch('{{ route("notifications.mark-all-read") }}', { method: 'POST' })
                    .then(function() {
                        loadUserNotifications();
                        updateUserNotificationBadge();
                    });
