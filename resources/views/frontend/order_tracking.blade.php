...
@section('title', 'Track Order - Pet Animixon')
...
<div class="track-page">
    <div class="track-header">
        ...
        <div class="track-id-badge">Order {{ $order->order_id }}</div>
    </div>
        ...
        <div class="map-eta-bar">
            ...
            <div class="eta-item">
                ...
                <div class="eta-sub" id="etaStatus">
                    @if($order->order_status === 'shipped') In Transit
        ...
        <div class="track-timeline">
            @foreach($statuses as $key => $label)
                ...
                <div class="track-step {{ $isDone ? 'done' : '' }} {{ $isCurrent ? 'current' : '' }}">
                    <div class="track-dot">{{ $isDone ? '✓' : ($isCurrent ? '●' : '') }}</div>
                    <div class="track-step-title">{{ $label }}</div>
                    <div class="track-step-desc">{{ $statusDescs[$key] }}</div>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="track-card">
        ...
        <div class="track-detail-row">
            ...
            <span class="track-detail-value">{{ $order->created_at->format('M j, Y g:i A') }}</span>
        </div>
        <div class="track-detail-row">
            <span class="track-detail-label">Status</span>
            <span class="track-detail-value status status-{{ $order->order_status }}">{{ ucfirst($order->order_status) }}</span>
        </div>
        <div class="track-detail-row">
            <span class="track-detail-label">Payment</span>
            <span class="track-detail-value">{{ $order->payment_method === 'cod' ? 'Cash on Delivery' : 'GCash' }}</span>
        </div>
        <div class="track-detail-row">
            <span class="track-detail-label">Shipping</span>
            <span class="track-detail-value">{{ ucfirst($order->shipping_method ?? 'Standard') }} Delivery</span>
        </div>
        @if($order->tracking_number)
        <div class="track-detail-row">
            <span class="track-detail-label">Tracking #</span>
            <span class="track-detail-value">{{ $order->tracking_number }}</span>
        </div>
        @endif
        @if($order->shippingAddress)
        <div class="track-detail-row">
            <span class="track-detail-label">Deliver to</span>
            <span class="track-detail-value">{{ $order->shippingAddress->recipient_name }}, {{ $order->shippingAddress->street_address }}, {{ $order->shippingAddress->city_municipality }}</span>
        </div>
        @endif
    </div>
    ...
    <div class="track-card">
        ...
        <div class="track-detail-row"><span class="track-detail-label">Subtotal</span><span class="track-detail-value">₱{{ number_format($order->order_amount, 2) }}</span></div>
        <div class="track-detail-row"><span class="track-detail-label">Shipping</span><span class="track-detail-value">{{ $order->shipping_fee > 0 ? '₱'.number_format($order->shipping_fee, 2) : 'FREE' }}</span></div>
        @if($order->discount_amount > 0)
        <div class="track-detail-row"><span class="track-detail-label" style="color:#3DB868;">Discount</span><span class="track-detail-value" style="color:#3DB868;">-₱{{ number_format($order->discount_amount, 2) }}</span></div>
        @endif
        <div class="track-detail-row" style="font-size:16px;font-weight:800;border-top:2px solid #eee;padding-top:12px;margin-top:4px;">
            <span>Total</span><span style="color:#FF8C42;">₱{{ number_format($order->total_amount, 2) }}</span>
        </div>
    </div>
...
(function() {
    ...
    @endphp
    const DEST = { lat: {{ $destLat }}, lng: {{ $destLng }}, label: '{{ addslashes($destLabel) }}' };
    ...
    if (isShipped || isDelivered) {
        ...
        if (isDelivered) {
            ...
            const orderAge = (Date.now() - new Date('{{ $order->updated_at->toISOString() }}').getTime()) / 1000;
    ...
    function sendNotification() {
        new Notification('🚚 Pet Animixon — Order On The Way!', {
            body: 'Your order {{ $order->order_id }} has been shipped and is heading your way. Track it live on the tracking page!',
            icon: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">🐾</text></svg>',
            tag: 'order-shipped-{{ $order->order_id }}',
