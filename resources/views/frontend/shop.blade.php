...
@section('title', 'Shop - Pet Markt-PH')
...
@push('styles')
    <style>
        ...
        .products-grid {
            ...
            .product-price {
                font-size: 18px; font-weight: 800; color: var(--ud-orange);
            }
...
@section('content')
    ...
    {{-- ══════════ Two-Column Hero: Featured Products + Brands ══════════ --}}
    <div class="shop-top-section">
        <div class="shop-top-grid">

            {{-- LEFT: Featured / Popular Products --}}
            <div class="shop-top-col hero-banner-col" style="padding: 0; border: none; background: transparent;">
                <style>
                    ...
                    .hero-price-current { font-size: 1.6rem; font-weight: 700; color: #ffffff; }
                    ...
                    .hero-right img {
                        max-height: 280px; max-width: 100%; width: auto; height: auto;
                ...
                <div class="hero-banner" id="splitHeroBanner">
                    ...
                    @foreach($featuredProducts->take(5) as $index => $fp)
                        @php
                        ...
                        @endphp
                        <div class="hero-slide {{ $index === 0 ? 'active' : '' }}" data-bg="{{ $bg }}" style="{{ $index === 0 ? 'background: ' . $bg . '; opacity: 1;' : '' }}">
                            <div class="hero-left">
                                <span class="hero-badge">{{ $badgeText }}</span>
                                <h2 class="hero-product-name">{{ $fp->product_name }}</h2>
                                <p class="hero-description">{{ $fp->short_description ?: 'High quality pet product for your awesome companion.' }}</p>

                                <div class="hero-price">
                                    <span class="hero-price-current">₱{{ number_format((float) $salePrice, 2) }}</span>
                                    @if($isOnSale)
                                        <span class="hero-price-original">₱{{ number_format((float) $fp->price, 2) }}</span>
                                    @endif
                                </div>

                                @auth
                                    <form action="{{ route('cart.add') }}" method="POST" onclick="event.stopPropagation();" class="hero-cart-form">
                                        @csrf
                                        <input type="hidden" name="product_id" value="{{ $fp->id }}">
                                        <input type="hidden" name="quantity" value="1">
                                        <button type="submit" class="hero-cta" {{ $fp->stock > 0 ? '' : 'disabled' }}>
                                            {{ $fp->stock > 0 ? 'Add to Cart Now! →' : 'Out of Stock' }}
                                        </button>
                                    </form>
                                @else
                                    <a href="{{ route('login') }}" class="hero-cta" onclick="event.stopPropagation();">
                                        {{ $fp->stock > 0 ? 'Add to Cart Now! →' : 'Out of Stock' }}
                                    </a>
                                @endauth
                            </div>
                            <div class="hero-right" onclick="window.openProductModal({{ $fp->id }}, event); cursor:pointer;">
                                <img src="{{ $fp->image_url }}" alt="{{ $fp->product_name }}">
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="shop-top-col">
                ...
                <div class="brand-list-grid">
                    @foreach($brands as $brand)
                        <a href="{{ route('shop.all', ['brand[]' => $brand->name]) }}" class="brand-card-mini">
                    ...
                    <div class="product-card">
                        <div class="product-img-wrap">
                            ...
                            @if($product->is_sale_active)
                                <span class="product-badge" style="background: #ef4444; top: auto; bottom: 10px;">{{ $product->discount_type === 'percent' ? '-' . (int)$product->discount_amount . '%' : 'SALE' }}</span>
                            @endif
                        </div>

                        <div class="product-body">
                            <div style="font-size: 11px; color: #888; text-transform: uppercase; font-weight: 700; margin-bottom: 2px;">{{ $product->brand_name }}</div>
                            <h3 class="product-name" style="min-height: auto; margin-bottom: 4px; cursor:pointer; transition:color 0.2s;" onmouseover="this.style.color='var(--ud-orange)'" onmouseout="this.style.color='inherit'" onclick="window.openProductModal({{ $product->id }}, event)">{{ $product->product_name }}</h3>
                            ...
                            <div class="product-rating">
                                <span class="stars" style="position: relative; display: inline-block; color: #d1d5db; letter-spacing: 2px;" title="{{ number_format($pReviewAvg, 1) }} out of 5">
                                    ★★★★★
                                    <div style="position: absolute; top:0; left:0; width: {{ ($pReviewAvg / 5) * 100 }}%; overflow: hidden; color: #f59e0b; white-space: nowrap;">★★★★★</div>
                                </span>
                            ...
                            <div class="product-price-row">
                                @if($product->is_sale_active)
                                    <span class="product-price">₱{{ number_format((float) $product->sale_price, 2) }}</span>
                                    <span class="product-original">₱{{ number_format((float) $product->price, 2) }}</span>
                                @else
