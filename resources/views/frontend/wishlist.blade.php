...
<div class="wl-page">
    <div class="wl-header">
        <h1 class="wl-heading">My <span>Wishlist</span> <span class="wl-count">{{ $items->count() }}</span></h1>
    ...
    @if($items->isEmpty())
        <div class="wl-empty">
            ...
            <a href="{{ route('shop.all') }}" class="wl-btn wl-btn-cart" style="display: inline-flex; padding: 12px 24px;">Browse Products</a>
        </div>
    @else
        <div class="wl-grid">
            @foreach($items as $item)
                @if($item->product)
                <div class="wl-card">
                    <a href="{{ route('product.show', $item->product->id) }}" class="js-open-product-modal" data-product-id="{{ $item->product->id }}" style="text-decoration: none; color: inherit;">
                        <div class="wl-img-wrap">
                            <img src="{{ $item->product->image_url }}" alt="{{ $item->product->product_name }}" class="wl-img">
                        </div>
                    </a>
                    <div class="wl-body">
                        ...
                        <div class="wl-actions">
                            ...
                            <a href="{{ route('wishlist.remove', $item->id) }}" class="wl-btn wl-btn-remove" onclick="return confirm('Remove from wishlist?')">✕</a>
                        </div>
                    </div>
                </div>
                @endif
