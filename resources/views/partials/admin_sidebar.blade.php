...
<aside class="sidebar">
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('admin.orders') }}" class="nav-item {{ $current==='admin.orders' ? 'active':'' }}" data-page="orders">
        ...
        <a href="{{ route('inventory.admin') }}" class="nav-item {{ $current==='inventory.admin' ? 'active':'' }}" data-page="inventory">
            <span class="nav-icon"><svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z"/></svg></span>
            <span class="nav-label">Inventory</span>
        </a>
        
        {{-- Only Main Admin can access customers --}}
        @if($isMainAdmin)
        ...
        {{-- Main Admin & Supervisor can access analytics --}}
        @if($isSupervisor)
        ...
        {{-- Main Admin & Supervisor can manage categories --}}
        @if($isSupervisor)
        ...
        {{-- Main Admin & Supervisor can manage brands --}}
        @if($isSupervisor)
        ...
        {{-- Only Main Admin can access revenue/financial reports --}}
        @if($isMainAdmin)
        ...
        @if($isMainAdmin)
        <a href="{{ route('settings.admin') }}" class="nav-item {{ $current==='settings.admin' ? 'active':'' }}" data-page="settings">
        ...
        {{-- Main Admin & Supervisor can manage admin users --}}
        @if($isSupervisor)
        ...
        {{-- Only Main Admin can approve admin requests --}}
        @if($isMainAdmin)
        <a href="{{ route('admin.requests') }}" class="nav-item {{ $current==='admin.requests' ? 'active':'' }}" data-page="admin-requests">
