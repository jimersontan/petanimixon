...
<div class="me-page-container">
    {{-- Header --}}
    <div class="me-header">
        <img src="{{ $user->profile_picture_url }}" alt="{{ $user->first_name }}" class="me-avatar" onerror="this.src='{{ asset('images/default_avatar.png') }}'">
        <div class="me-user-info">
            <h2 class="me-user-name">{{ $user->first_name }} {{ $user->last_name }}</h2>
            <p class="me-user-email">{{ $user->email }}</p>
        </div>
        <a href="{{ route('profile.edit') }}" style="color: white; opacity: 0.9;" aria-label="Settings">
    ...
    <div class="me-section">
        <div class="me-section-header">
            ...
            <a href="{{ route('orders') }}" class="me-section-link">
                View Purchase History
                <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
            </a>
        </div>
        <div class="me-purchases-grid">
            <a href="{{ route('orders') }}?status=pending" class="me-purchase-item">
                @if(($orderStats['pending'] ?? 0) > 0)
                    <div class="me-badge">{{ $orderStats['pending'] > 99 ? '99+' : $orderStats['pending'] }}</div>
                @endif
            ...
            <a href="{{ route('orders') }}?status=processing" class="me-purchase-item">
                @if(($orderStats['processing'] ?? 0) > 0)
                    <div class="me-badge">{{ $orderStats['processing'] > 99 ? '99+' : $orderStats['processing'] }}</div>
                @endif
            ...
            <a href="{{ route('orders') }}?status=out_for_delivery" class="me-purchase-item">
                @if(($orderStats['out_for_delivery'] ?? 0) > 0)
                    <div class="me-badge">{{ $orderStats['out_for_delivery'] > 99 ? '99+' : $orderStats['out_for_delivery'] }}</div>
                @endif
            ...
            <a href="{{ route('orders') }}?status=delivered" class="me-purchase-item">
                @if(($orderStats['delivered'] ?? 0) > 0)
                    <div class="me-badge">{{ $orderStats['delivered'] > 99 ? '99+' : $orderStats['delivered'] }}</div>
                @endif
    ...
    <div class="me-section" style="margin-top: 12px;">
        <div class="me-options-list">
            <a href="{{ route('wishlist.index') }}" class="me-option-item">
            ...
            <a href="{{ route('profile.edit') }}" class="me-option-item">
