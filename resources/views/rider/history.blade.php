...
<head>
    ...
    <title>Delivery History - Pet Markt-PH Rider</title>
    ...
    <div class="rider-layout">
        <aside class="rider-sidebar">
            ...
            <nav>
                ...
                <a href="{{ route('rider.history') }}" class="nav-item active" data-page="history">
        ...
        <main class="rider-main">
            ...
            <div class="rider-stats-grid">
                ...
                <div class="rider-stat-card">
                    ...
                    <div>
                        <div class="rider-stat-value">{{ $stats['this_week'] }}</div>
                ...
                <div class="rider-stat-card">
                    ...
                    <div>
                        <div class="rider-stat-value">{{ $stats['this_month'] }}</div>
            ...
            <div class="rider-filter-bar">
                <form method="get" action="{{ route('rider.history') }}" style="display:flex; gap:10px; align-items:center;">
            ...
            <div class="rider-card">
                ...
                @if($orders->count() > 0)
                    ...
                    <div class="delivery-history-item">
                        <div class="delivery-history-icon {{ $order->order_status === 'delivered' ? 'delivered' : 'cancelled' }}">
                            {{ $order->order_status === 'delivered' ? '✅' : '❌' }}
                        </div>
                        <div class="delivery-history-info">
                            <div class="delivery-history-id">{{ $order->display_id }}</div>
                            <div class="delivery-history-customer">
                                {{ optional($order->user)->full_name ?? 'Customer' }}
                                · {{ optional($order->shippingAddress)->city_municipality ?? '' }}
                            </div>
                            @if($order->rider_notes)
                        ...
                        <div style="text-align: right;">
                            <div class="delivery-history-total">{{ $order->formatted_total }}</div>
                            <div class="delivery-history-date">
                                {{ optional($order->rider_delivered_at ? \Carbon\Carbon::parse($order->rider_delivered_at) : $order->updated_at)->format('M j, Y g:i A') }}
                            </div>
                            <span class="rider-badge-status rider-badge-{{ $order->order_status }}">{{ ucfirst(str_replace('_', ' ', $order->order_status)) }}</span>
                        </div>
                    </div>
                    @endforeach
