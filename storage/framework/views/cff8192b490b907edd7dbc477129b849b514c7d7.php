            ...
            <div id="adminNotificationPanel" class="ud-notification-panel" style="
                ...
                <div style="
                    ...
                    <button type="button" id="adminMarkAllRead" style="
                        background: none; border: none; color: var(--ud-orange-dark); font-size: 12px;
...
<style>
    ...
    .ud-toast {
        ...
        border-left: 4px solid var(--ud-orange-dark);
        position: relative;
        z-index: 9999;
    }
    .ud-toast.show {
...
<script>
    ...
    function updateAdminNotificationBadge() {
        fetch('<?php echo e(route("notifications.unread")); ?>', { headers: csrfHeaders() })
            .then(r => r.json())
            .then(data => {
                const badge = document.getElementById('notificationBadge');
                if (data.unread_count > 0) {
    ...
    function loadAdminNotifications() {
        fetch('<?php echo e(route("notifications.list")); ?>?limit=10', { headers: csrfHeaders() })
            .then(r => r.json())
            .then(data => {
                ...
                if (data.data && data.data.length > 0) {
                    list.innerHTML = data.data.map(n => {
                        return '<div onclick="window.handleAdminNotifClick(' + n.id + ')" ' +
                            ...
                            'border-radius: 6px; border-left: 4px solid var(--ud-orange-dark); cursor: pointer; transition: background 0.2s;">' +
    ...
    document.getElementById('adminMarkAllRead').addEventListener('click', function(e) {
        e.stopPropagation();
        fetch('<?php echo e(route("notifications.mark-all-read")); ?>', { method: 'POST', headers: csrfHeaders() })
            .then(() => {
                loadAdminNotifications();
                updateAdminNotificationBadge();
            });
