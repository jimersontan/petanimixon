    ...
    <div class="pd-info">
        ...
        <?php if(auth()->guard()->check()): ?>
            <div style="display: flex; gap: 10px; margin-top: 15px; align-items: center; flex-wrap: wrap;">
                <form action="<?php echo e(route('cart.add')); ?>" method="POST" class="pd-add-form pd-add-cart-form" style="margin: 0; flex: 1; display:flex;">
                    <?php echo csrf_field(); ?>
                    ...
                    <div style="display: flex; gap: 10px; width: 100%;">
                        <div class="pd-qty-wrap" style="height: 48px;">
                            ...
                            <input type="number" name="quantity" value="1" min="1" max="<?php echo e($product->stock); ?>" class="pd-qty-input" style="height: 100%;" readonly>
                        ...
                        <button type="submit" class="pd-add-btn" style="height: 48px; border-radius: 10px;" <?php echo e($product->stock < 1 ? 'disabled style="opacity:.5;cursor:not-allowed;"' : ''); ?>>🛒 <?php echo e($product->stock < 1 ? 'Out of Stock' : 'Add to Cart'); ?></button>
                    </div>
                </form>
                ...
                <form action="<?php echo e(route('wishlist.toggle')); ?>" method="POST" style="margin: 0; height: 48px;">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="product_id" value="<?php echo e($product->id); ?>">
                    <button type="submit" style="height: 100%; width: 48px; border-radius: 10px; background: #fff1f2; border: 1px solid #fecdd3; color: #e11d48; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" title="Toggle Wishlist" onmouseover="this.style.background='#ffe4e6'" onmouseout="this.style.background='#fff1f2'">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="<?php echo e((Auth::check() && \App\Models\Wishlist::where('user_id', Auth::id())->where('product_id', $product->id)->exists()) ? 'currentColor' : 'none'); ?>" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
                    </button>
                </form>
            </div>
        <?php else: ?>
            <div style="display: flex; gap: 10px; margin-top: 15px; align-items: center;">
                <a href="<?php echo e(route('login')); ?>" class="pd-add-btn" style="flex: 1; text-align: center; text-decoration: none; display: flex; align-items: center; justify-content: center; box-sizing: border-box; height: 48px;">🛒 Add to Cart</a>
                <?php if($product->stock > 0): ?>
                <a href="<?php echo e(route('login')); ?>" class="pd-add-btn" style="flex: 1; background: #3b82f6; text-align: center; text-decoration: none; display: flex; align-items: center; justify-content: center; box-sizing: border-box; height: 48px;">⚡ Buy Now</a>
                <?php endif; ?>
                <a href="<?php echo e(route('login')); ?>" style="height: 48px; width: 48px; border-radius: 10px; background: #fff1f2; border: 1px solid #fecdd3; color: #e11d48; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" title="Add to Wishlist">
